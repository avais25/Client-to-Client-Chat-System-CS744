...
void socket_creator(long s_port)
	...
	while(bind(server_socket, (struct sockaddr*) &server_address, sizeof(server_address))<0){
		system("fuser -k 9001/tcp");
		printf("Bind failed at server..Retrying.......\n");
		if(bindCount<=0)
		return ;
		else
			bindCount--;
...
/* Try a GET and two INCR */
 ...
 //    printf("%s\n", getQ);
   ...
   //int res=0;
    ...
    reply = redisCommand(c,"EXISTS %s",userId);
    printf("it may be exist  : %lld \n", reply->integer);
	...
	if(res==1){
	    ...
	    reply = redisCommand(c,"LRANGE %s 0 1",userId);
		...
		strcpy(epass,reply->element[1]->str);
	    printf("%s\n",epass );  
		freeReplyObject(reply);

		printf("%s --------%s\n",epass,upassword );

	    if(strcmp(epass,upassword)==0){
	    	//printf("user exist\n");    
	    	flag=1;
	    	printf("yag\n");
	...
	switch(flag)			//query flag 0=invalid uid ;;; 1=Success ;;; 2=Wrong password
	{
...
void regServer(long accept_socket)			//this function maps with regClient in the client code, it implements Registeration
{
	...
	*********************************************************************/
    ...
    if(res==0){
	    ...
	    /* Try a GET and two INCR */
	   reply = redisCommand(c,"GET currPort");
	    //int portNO=atoi(reply->str);
	   // printf("GET foo: %s\n", reply->str);
	    //printf("port no is %d\n",portNO );
	    char portNoStr[10]="";
	    strcpy(portNoStr,reply->str);
	    freeReplyObject(reply);

	    reply = redisCommand(c,"INCR currPort");
	   printf("INCR counter: %lld\n", reply->integer);
	    freeReplyObject(reply);


	    /* Create a list of numbers, with name as user id and atrribute as port no(0), password(1)*/
	    reply = redisCommand(c,"DEL %s",userId);
	    freeReplyObject(reply);
	    char pushQ[100]="PUSH ";
	    strcat(pushQ,userId);
	    strcat(pushQ," ");
		strcat(pushQ,upassword);
	    strcat(pushQ," ");
	    strcat(pushQ,portNoStr);
	    
	    printf("%s\n",pushQ );

	    reply = redisCommand(c,"LPUSH %b %b %b", userId,(size_t)strlen(userId),upassword,(size_t)strlen(upassword),portNoStr,(size_t)strlen(portNoStr));
	    freeReplyObject(reply);

	   
	     /* Let's check what we have inside the list */
	    reply = redisCommand(c,"LRANGE %s 0 3",userId);
	    if (reply->type == REDIS_REPLY_ARRAY) {
	    	printf("%s\n", userId);
	        for (j = 0; j < reply->elements; j++) {
	            printf("%u) %s\n", j, reply->element[j]->str);
    ...
    // reply = redisCommand(c,"SET %b %b",userId, (size_t) strlen(userId), upassword, (size_t) strlen(upassword));
